...
{
    public partial class frm_mange_part_stud : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        void get_data()
        {
            ...
            dbContext.TBL_SEND_STUD_SPEC.LoadAsync().ContinueWith(loadTask =>
            {
                // Bind data to control when loading complete
                gridControl1.DataSource = dbContext.TBL_SEND_STUD_SPEC.Local.ToBindingList();
        ...
        {
            FRM_ADD_stud_part FRM = new FRM_ADD_stud_part();
            FRM.SEND_STUD_ID  = 0;
        ...
        void delete()
        {
            if (SEND_STUD_ID  != 0)
            {
                try
                {
                    ...
                    {
                        TBL_SEND_STUD_SPEC del_cle = con.TBL_SEND_STUD_SPEC.Find(SEND_STUD_ID );
                        // del_doc.DOC_ID = holiday_type_id;

                        con.TBL_SEND_STUD_SPEC.Remove(del_cle);
                        con.SaveChanges();
                        tost not = new tost();
        ...
        void get_sele()
        {
            if (gridView2.RowCount > 0)
            {
                ...
                SEND_STUD_ID  = Convert.ToInt32(gridView2.GetFocusedRowCellValue("SEND_STUD_ID").ToString());
